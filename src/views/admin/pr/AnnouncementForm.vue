        ...
        <div class="rounded-t bg-white mb-0 px-6 py-6">
          <div class="text-center flex justify-between items-center">
            <h6 class="text-blueGray-700 text-xl font-bold">
              {{ isEditing ? 'แก้ไขข้อมูลประกาศโรงเรียน' : 'เพิ่มข้อมูลประกาศโรงเรียน' }}
            </h6>
            <div>
              <router-link
...
export default {
  ...
  methods: {
    async fetchData() {
      try {
        const docRef = doc(db, "announcements", this.docId);
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
          this.form = docSnap.data();
        } else {
          alert("ไม่พบข้อมูลข่าวนี้");
          this.$router.push("/admin/pr/announcements");
        }
      } catch (error) {
        ...
        if (files.length > 0) {
          ...
          reader.onload = async (event) => {
            try {
              ...
              this.cropImageSrc = canvas.toDataURL('image/jpeg', 0.85);
    ...
    async saveData() {
      if (this.form.publishDate) {
        ...
        this.form.title = "ประกาศฉบับวันที่ " + thaiDateStr;
      }
      ...
      try {
        if (this.isEditing) {
          ...
          await addDoc(collection(db, "announcements"), this.form);
        }
        this.$router.push("/admin/pr/announcements");
      } catch (error) {
