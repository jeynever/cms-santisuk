        ...
        <div class="rounded-t bg-white mb-0 px-6 py-6">
          <div class="text-center flex justify-between items-center">
            <h6 class="text-blueGray-700 text-xl font-bold">
              {{ isEditing ? 'แก้ไขข้อมูลวารสารโรงเรียน' : 'เพิ่มข้อมูลวารสารโรงเรียน' }}
            </h6>
            <div>
              <router-link
              ...
              <div class="w-full lg:w-4/12 px-4">
                <div class="relative w-full mb-3">
                  <label class="block uppercase text-blueGray-600 text-xs font-bold mb-2">
                    รูปภาพวารสาร (A4 แนวตั้ง , ไม่เกิน 2MB)
                  </label>
                  <input
                    type="file"
                    ref="fileInput"
                  ...
                  <div v-if="form.imageUrl" class="mt-4">
                    <img :src="form.imageUrl" class="w-1/2 rounded shadow" />
                  </div>
                </div>
              </div>
    ...
    <div v-if="showCropModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75">
      <div class="relative bg-white w-full max-w-4xl mx-4 rounded-lg shadow-lg flex flex-col max-h-screen">
        <div class="flex items-start justify-between p-5 border-b border-solid border-blueGray-200 rounded-t">
          <h3 class="text-xl font-semibold">ครอบตัดรูปภาพวารสาร (A4 แนวตั้ง)</h3>
...
export default {
  ...
  methods: {
    async fetchData() {
      try {
        const docRef = doc(db, "journals", this.docId);
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
          this.form = docSnap.data();
        } else {
          alert("ไม่พบข้อมูลข่าวนี้");
          this.$router.push("/admin/pr/journals");
        }
      } catch (error) {
    ...
    async cropImage() {
      ...
      const canvas = this.cropperInstance.getCroppedCanvas({
        width: 840, // A4 aspect width
        height: 1188, // A4 aspect height
      });
    ...
    async saveData() {
      if (this.form.publishDate) {
        const dateObj = new Date(this.form.publishDate);
        const thaiMonths = [
        ...
        this.form.title = "วารสารฉบับวันที่ " + thaiDateStr;
      }

      if (!this.form.publishDate || !this.form.imageUrl) {
        alert("กรุณากรอกวันที่และอัปโหลดรูปภาพให้ครบถ้วน");
        return;
      }
      this.isSaving = true;
      try {
        if (this.isEditing) {
          ...
          await addDoc(collection(db, "journals"), this.form);
        }
        this.$router.push("/admin/pr/journals");
      } catch (error) {
