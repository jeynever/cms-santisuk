        ...
        <div class="rounded-t bg-white mb-0 px-6 py-6">
          <div class="text-center flex justify-between items-center">
            <h6 class="text-blueGray-700 text-xl font-bold">
              {{ isEditing ? 'แก้ไขข้อมูลผลงานโรงเรียน' : 'เพิ่มข้อมูลผลงานโรงเรียน' }}
            </h6>
            <div>
              <router-link
    ...
    <div v-if="showCropModal" class="fixed inset-0 z-50 flex items-start justify-center bg-black bg-opacity-75 pt-10">
      <div class="relative bg-white w-full max-w-4xl mx-4 rounded-lg shadow-lg flex flex-col max-h-screen">
        <div class="flex items-start justify-between p-5 border-b border-solid border-blueGray-200 rounded-t">
          <h3 class="text-xl font-semibold">ครอบตัดรูปภาพผลงาน (A4 แนวนอน)</h3>
...
export default {
  ...
  methods: {
    async fetchData() {
      try {
        const docRef = doc(db, "achievements", this.docId);
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
          this.form = docSnap.data();
        } else {
          alert("ไม่พบข้อมูลข่าวนี้");
          this.$router.push("/admin/pr/achievements");
        }
      } catch (error) {
    ...
    async cropImage() {
      ...
      const canvas = this.cropperInstance.getCroppedCanvas({
        width: 1188, // A4 aspect width
        height: 840, // A4 aspect height
      });
    ...
    async saveData() {
      if (!this.form.title || !this.form.publishDate || !this.form.imageUrl) {
        alert("กรุณากรอกชื่อผลงาน วันที่ และอัปโหลดไฟล์ให้ครบถ้วน");
        return;
      }
      this.isSaving = true;
      try {
        if (this.isEditing) {
          ...
          await addDoc(collection(db, "achievements"), this.form);
        }
        this.$router.push("/admin/pr/achievements");
      } catch (error) {
