    ...
    <div class="block w-full overflow-x-auto">
      <table class="items-center w-full bg-transparent border-collapse">
        ...
        <tbody>
          ...
          <tr v-for="item in achievements" :key="item.id" v-else>
            <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4">
              <img :src="item.imageUrl" alt="Achievements Image" class="rounded shadow-sm" style="width: 40px; max-width: 40px; height: auto; object-fit: contain;" v-if="item.imageUrl" />
            ...
            <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left">
              <router-link
                :to="`/admin/pr/achievements/edit/${item.id}`"
  ...
  methods: {
    async fetchAchievements() {
      ...
      try {
        ...
        const q = query(collection(db, "achievements"), orderBy("publishDate", "desc"));
